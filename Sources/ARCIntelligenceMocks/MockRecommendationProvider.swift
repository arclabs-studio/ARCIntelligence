...
public final class MockRecommendationProvider: RecommendationProvider, Sendable {
    ...
    public func complete(prompt: String,
        ...
        return IntelligenceResponse(content: "Mock completion for recommendation provider",
    ...
    public func generateRecommendations(for _: some Codable & Sendable,
                                        count: Int,
                                        configuration _: RecommendationConfiguration) async throws -> [Recommendation] {
        try await Task.sleep(for: .seconds(simulatedDelay))
