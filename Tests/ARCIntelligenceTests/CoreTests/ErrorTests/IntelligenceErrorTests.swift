...
@Suite("IntelligenceError Tests", .tags(.unit)) struct IntelligenceErrorTests {
    @Test("Provider unavailable error has correct description") func providerUnavailableErrorDescription() {
        let error = IntelligenceError.providerUnavailable
        #expect(error.errorDescription?.contains("not available") == true)
    }

    @Test("Provider not configured error includes name") func providerNotConfiguredErrorIncludesName() {
    ...
    @Test("Invalid request error includes reason") func invalidRequestErrorIncludesReason() {
    ...
    @Test("Token limit exceeded error includes counts") func tokenLimitExceededErrorIncludesCounts() {
